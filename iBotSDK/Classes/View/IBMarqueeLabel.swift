...
open class IBMarqueeLabel: UILabel, CAAnimationDelegate {
     ...
     */
    open var type: IBMarqueeType = .continuous {
        didSet {
            if type == oldValue {
                return
            }
    ...
    /**
     ...
     - SeeAlso: FadeStep
     */
    open var scrollSequence: Array<IBMarqueeStep>?
    ...
    /**
     ...
     - SeeAlso: SpeedLimit
     */
    open var speed: IBSpeedLimit = .duration(7.0) {
        didSet {
    ...
    */
    ...
    private func setup() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(IBMarqueeLabel.restartForViewController(_:)), name: NSNotification.Name(rawValue: IBMarqueeKeys.Restart.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(IBMarqueeLabel.labelizeForController(_:)), name: NSNotification.Name(rawValue: IBMarqueeKeys.Labelize.rawValue), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(IBMarqueeLabel.animateForController(_:)), name: NSNotification.Name(rawValue: IBMarqueeKeys.Animate.rawValue), object: nil)
    ...
    private func updateAndScroll(overrideHold: Bool) {
        ...
        case .continuous, .continuousReverse:
            ...
            sequence = scrollSequence ?? [
                IBScrollStep(timeStep: 0.0, position: .home, edgeFades: .trailing),                   // Starting point, at home, with trailing fade
                IBScrollStep(timeStep: animationDelay, position: .home, edgeFades: .trailing),        // Delay at home, maintaining fade state
                IBFadeStep(timeStep: 0.2, edgeFades: [.leading, .trailing]),                          // 0.2 sec after scroll start, fade leading edge in as well
                IBFadeStep(timeStep: (startFadeTime - animationDuration),                             // Maintain fade state until just before reaching end of scroll animation
                         edgeFades: [.leading, .trailing]),
                IBScrollStep(timeStep: animationDuration, timingFunction: animationCurve,             // Ending point (back at home), with animationCurve transition, with trailing fade
        ...
        case .leftRight, .left, .rightLeft, .right:
            ...
            if (type == .leftRight || type == .rightLeft) {
                sequence = scrollSequence ?? [
                    IBScrollStep(timeStep: 0.0, position: .home, edgeFades: .trailing),               // Starting point, at home, with trailing fade
                    IBScrollStep(timeStep: animationDelay, position: .home, edgeFades: .trailing),    // Delay at home, maintaining fade state
                    IBFadeStep(timeStep: 0.2, edgeFades: [.leading, .trailing]),                      // 0.2 sec after delay ends, fade leading edge in as well
                    IBFadeStep(timeStep: -0.2, edgeFades: [.leading, .trailing]),                     // Maintain fade state until 0.2 sec before reaching away position
                    IBScrollStep(timeStep: animationDuration, timingFunction: animationCurve,         // Away position, using animationCurve transition, with only leading edge faded in
                        position: .away, edgeFades: .leading),
                    IBScrollStep(timeStep: animationDelay, position: .away, edgeFades: .leading),     // Delay at away, maintaining fade state (leading only)
                    IBFadeStep(timeStep: 0.2, edgeFades: [.leading, .trailing]),                      // 0.2 sec after delay ends, fade trailing edge back in as well
                    IBFadeStep(timeStep: -0.2, edgeFades: [.leading, .trailing]),                     // Maintain fade state until 0.2 sec before reaching home position
                    IBScrollStep(timeStep: animationDuration, timingFunction: animationCurve,         // Ending point, back at home, with only trailing fade
                ...
                sequence = scrollSequence ?? [
                    ...
                    IBScrollStep(timeStep: animationDuration, timingFunction: animationCurve,         // Away position, using animationCurve transition, with only leading edge faded in
                        position: .away, edgeFades: .leading),
                    IBScrollStep(timeStep: animationDelay, position: .away, edgeFades: .leading),     // "Delay" at away, maintaining fade state
                ]
            }
        }
    ...
    private func beginScroll(_ sequence: [IBMarqueeStep]) {
    ...
    private func scroll(_ scroller: MLAnimation, fader: MLAnimation?) {
        ...
        scroller.anim.setValue(true, forKey: IBMarqueeKeys.CompletionClosure.rawValue)
        scroller.anim.delegate = self
        if type == .left || type == .right {
            // Make it stay at away permanently
            scroller.anim.isRemovedOnCompletion = false
            scroller.anim.fillMode = kCAFillModeForwards   //.forwards
        }
    ...
    private func generateScrollAnimation(_ sequence: [IBMarqueeStep]) -> MLAnimation {
        ...
        let scrollSteps = sequence.filter({ $0 is IBScrollStep }) as! [IBScrollStep]
    ...
    private func generateGradientAnimation(_ sequence: [IBMarqueeStep], totalDuration: CGFloat) -> MLAnimation {
        // Setup
        var totalTime: CGFloat = 0.0
        ...
        let fadeSteps = sequence.enumerated().filter { (arg: (offset: Int, element: IBMarqueeStep)) -> Bool in
            let (offset, element) = arg
            
            // Include all Scroll Steps
            if element is IBScrollStep { return true }
            ...
            let subsequent = element.timeStep >= 0 && (sequence[max(0, offset - 1)] is IBScrollStep)
            // Precedent step if 1) negative time step and 2) precedes a Scroll Step
            let precedent = element.timeStep < 0 && (sequence[min(sequence.count - 1, offset + 1)] is IBScrollStep)
    ...
    private func applyGradientMask(_ fadeLength: CGFloat, animated: Bool, firstStep: IBMarqueeStep? = nil) {
        ...
        if (animated) {
            ...
            let colorAnimation = IBGradientSetupAnimation(keyPath: "colors")
            colorAnimation.fromValue = gradientMask.colors
            colorAnimation.toValue = adjustedColors
            colorAnimation.fillMode = kCAFillModeForwards //.forwards
...
//        return CAMediaTimingFunction(name: timingFunction!)
        ...
        switch curve {
        case .easeIn:
            timingFunction = kCAMediaTimingFunctionEaseIn as CAMediaTimingFunctionName  //.easeIn
        case .easeInOut:
            timingFunction = kCAMediaTimingFunctionEaseInEaseOut as CAMediaTimingFunctionName //.easeInEaseOut
        case .easeOut:
            timingFunction = kCAMediaTimingFunctionEaseOut as CAMediaTimingFunctionName //.easeOut
        default:
            timingFunction = kCAMediaTimingFunctionLinear as CAMediaTimingFunctionName //.linear
        }
        
        return CAMediaTimingFunction(name: timingFunction as String)
    }
    
    private func transactionDurationType(_ labelType: IBMarqueeType, interval: CGFloat, delay: CGFloat) -> TimeInterval {
        switch (labelType) {
    ...
    public func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        if let setupAnim = anim as? IBGradientSetupAnimation {
    ...
    fileprivate enum IBMarqueeKeys: String {
    ...
    class fileprivate func notifyController(_ controller: UIViewController, message: IBMarqueeKeys) {
 ...
 */
public struct IBScrollStep: IBMarqueeStep {
    /**
    ...
    */
    ...
    /**
     ...
     - SeeAlso: Position
     */
    public let position: IBPosition
    ...
    */
    public let edgeFades: IBEdgeFade
    
    public init(timeStep: CGFloat, timingFunction: UIView.AnimationCurve = .linear, position: IBPosition, edgeFades: IBEdgeFade) {
 ...
 */
public struct IBFadeStep: IBMarqueeStep {
    /**
    ...
    public init(timeStep: CGFloat, timingFunction: UIView.AnimationCurve = .linear, edgeFades: IBEdgeFade) {
        self.timeStep = timeStep
        self.timingFunction = timingFunction
        self.edgeFades = edgeFades
    }
...
public struct IBEdgeFade : OptionSet {
    public let rawValue: Int
    public static let leading  = IBEdgeFade(rawValue: 1 << 0)
    public static let trailing = IBEdgeFade(rawValue: 1 << 1)
...
fileprivate class IBGradientSetupAnimation: CABasicAnimation {
}
